...
#1) Load R image
# load("SARS-CoV-2_Modeling_20220525.RData") 
...
pdf(paste0(paste0("./Results/shotgun_relative_abundance_", format(Sys.Date(), format = "%Y%m%d"),".pdf")), height = 6, width = 12)
...
#### Alpha Diversity Plot START ####
row.names(mat) <- mat$patient_ID
mat <- mat %>% select(-patient_ID)

mat_t <- mat %>% t()

alpha <- vegan::diversity(mat,index="invsimpson") %>%
  as.data.frame()
colnames(alpha)[1] <- "InvSimpson"
alpha$patient_ID <- row.names(alpha)
...
gg_alpha <- alpha %>% 
  ...
  geom_pirate(cis_params = list(fill = "white", alpha = 0.5),
              ...
              cis = TRUE,
              violins = FALSE) +
  annotate("text", x = 1.1, y = 39.5, label = paste0("Wilcoxon, W = ", alpha_stats$statistic, ", p = ", alpha_stats$p),
           size = 4) +
  theme_bw() +
  theme(
    panel.grid = eb(),
...
pdf(paste0("./Results/shotgun_pirate_alpha_diversity_vital_", format(Sys.Date(), format = "%Y%m%d"),".pdf"), height = 6, width = 7)
gg_alpha
dev.off()
#### Alpha Diversity Plot END ####


#### UMAP Plot 1 START ####
custom_config <- umap.defaults
custom_config$n_neighbors <-  as.integer(nrow(mat) * 0.1)
custom_config$metric <-  'manhattan'

set.seed(6)
umap_mat <- umap(mat, config = custom_config)

umap_mat_plot <- umap_mat$layout %>% 
  as.data.frame() %>%
...
pdf(paste0("./Results/shotgun_otu_lefse_analysis_",format(Sys.Date(), format = "%Y%m%d"), ".pdf"), height = 8, width = 14)
...
pdf(paste0("./Results/Figure_1_",format(Sys.Date(), format = "%Y%m%d"), ".pdf"), height = 14, width = 14)
...
gridExtra::grid.arrange(
  ...
  gg_lefse_plot,           # 6
  layout_matrix = rbind(c(1,1,1,1, NA,  NA,NA,NA),
...
cairo_pdf(paste0("./Results/Figure_4A_", format(Sys.Date(), format = "%Y%m%d"), ".pdf"), width = 8, height = 6)
pROC_obj <- roc(
  cutpoints_results_var_slct$vital_status,
  cutpoints_results_var_slct$mmp_score,
  smoothed = TRUE,
...
```{r, survival-analysis, fig.dim=c(10,12), message=FALSE, warning=FALSE}
...
pdf(paste0("./Results/Figure_4B_", format(Sys.Date(), format = "%Y%m%d"),".pdf"), height = 6, width = 8, onefile = FALSE)
ggs
dev.off()
...
### Figure 5: Flow Chart
```{r, flow-chart, fig.dim=c(10,12), message=FALSE, warning=FALSE}
# Flow chart
ggstream <- covid_flow %>%
  mutate(
...
ggsave(filename = paste0("./Results/Figure_5_", format(Sys.Date(), format = "%Y%m%d"),".pdf"), ggstream, height = 10, width = 10)
```

### Figure 2A: CARD Analysis
```{r, card-genes, fig.dim=c(12,14), message=FALSE, warning=FALSE}
card_dict <- card_dict %>% 
...
pdf(paste0("./Results/card_heatmap_total_", format(Sys.Date(), format = "%Y%m%d"),".pdf"), height = 60, width = 16, onefile = F)
gg_card_heatmap
dev.off()
...
pdf(paste0("./Results/card_heatmap_family_", format(Sys.Date(), format = "%Y%m%d"),".pdf"), height = 8, width = 20, onefile = F)
draw(gg_card_family_heatmap_tot, auto_adjust = F)
dev.off()
```

### Figure 2C: Toxins Analysis
```{r, toxins-genes, fig.dim=c(12,14), message=FALSE, warning=FALSE}
...
pdf(paste0("./Results/toxins_heatmap_", format(Sys.Date(), format = "%Y%m%d"),".pdf"), height = 2.9, width = 16)
gg_toxin_heatmap_tot
dev.off()
```

### Figure 2B: Bacteriocins Analysis
```{r, bacteriocins-genes, fig.dim=c(12,14), message=FALSE, warning=FALSE}
...
pdf(paste0("./Results/bacteriocin_heatmap_total_", format(Sys.Date(), format = "%Y%m%d"),".pdf"), height = 6, width = 16, onefile = F)
gg_bactocin_heatmap_tot
dev.off()
```

### Figure 2D: BAI Operon Analysis
```{r, operon, fig.dim=c(10,12), message = FALSE, warning = FALSE}
#### Operon START ####
bai2 <- bai %>% 
...
pdf(paste0("./Results/operon_", format(Sys.Date(), format = "%Y%m%d"),".pdf"), height = 4, width = 6, onefile = F)
...
pdf(paste0("./Results/bacteriocins_toxins_card_heatmap_", format(Sys.Date(), format = "%Y%m%d"),".pdf"), height = 10, width = 14)
draw(ht_list, ht_gap = unit(1, "cm"))
dev.off()

pdf(paste0("./Results/unadjusted_heatmap_", format(Sys.Date(), format = "%Y%m%d"),".pdf"), height = 10, width = 14)
draw(unadj_list, ht_gap = unit(1, "cm"))
dev.off()

pdf(paste0("./Results/adjusted_heatmap_", format(Sys.Date(), format = "%Y%m%d"),".pdf"),  height = 10, width = 14)
draw(adj_list, ht_gap = unit(1, "cm"))
dev.off()
```

### Figure 3A: Volcano Plot
```{r, volcano-plot, fig.dim=c(10,8), message=FALSE, warning=FALSE}
...
ggsave(plot = volcano, paste0("./Results/volcano_plot_total_", format(Sys.Date(), format = "%Y%m%d"),".pdf"), width = 12, height = 8, units = "in")
```

### Figure 3B: Quant Metabolomics
```{r, quant-metabolomics, fig.dim=c(10,8), message=FALSE, warning=FALSE}
#### Metabolites START ####
...
metab_quant %>% 
  mutate(mvalue = case_when(compound == "deoxycholic acid" ~ (mvalue / deoxycholic_mw)*1000,
    ...
    compound = case_when(compound == "isodeoxycholic acid" ~ "Isodeoxycholic Acid", 
                         ...
                         compound == "indole-3-carboxaldehyde" ~ "Indole-3-Carboxaldehyde"
                              )) %>% 
...
quant_vital_indole2 <-
metab_quant %>% 
  mutate(compound = case_when(compound == "indole-3-carboxaldehyde" ~ "Indole-3-Carboxaldehyde")) %>% 
  ...
  geom_label(inherit.aes = F, aes(y = mvalue*1.2, x = 1.5, label = paste0("Wilcoxon,\n W = ", statistic, ",\n", "p.adj = ", p.adj)), 
             data = kw_metab_pvals %>% filter(compound %in% c("Indole-3-Carboxaldehyde")), 
             size = 3.25,
             alpha = 0.9) +
  theme_bw() +
  theme(panel.grid = eb(),
        ...
        legend.text = et(color = "black", size = 12)) +
  ggsci::scale_fill_igv()+
  facet_wrap(factor(compound, levels = c("Indole-3-Carboxaldehyde"))~., 
             scales = "free_y",
             ncol = 1
             ) + 
  ylab("") +
...
pdf(paste0("./Results/covid_quant_metab_vital_", format(Sys.Date(), format = "%Y%m%d"),".pdf"), height =4, width = 12)
quant_vital_tot
dev.off()
#### Metabolites END ####
```
...
pdf(paste0("./Results/Figure_3_", format(Sys.Date(), format = "%Y%m%d"),".pdf"), height = 14, width = 16)
...
write.csv(tab1_2, paste0("./Results/Table_1_", format(Sys.Date(), format = "%Y%m%d"),".csv"), row.names = TRUE)

# Need to adjust pvalues
tab1_2_padjust <- read.csv(paste0("./Results/Table_1_", format(Sys.Date(), format = "%Y%m%d"),".csv"))
...
write.csv(tab1_2_padjust, paste0("./Results/Table_1_padjust_", format(Sys.Date(), format = "%Y%m%d"),".csv"), row.names = TRUE)
```
...
tabletwo_vars <- c("InvSimpson", "lithocholic acid", "deoxycholic acid", "isodeoxycholic acid", "cholic acid",
                   ...
                   "butyrate", "propionate", "acetate", "succinate", "desaminotyrosine", "proteo_doms", "entero_doms")
...
write.csv(tab2_2, paste0("./Results/Table_2_", format(Sys.Date(), format = "%Y%m%d"),".csv"), row.names = TRUE)

# Need to adjust pvalues
tab2_2_padjust <- read.csv(paste0("./Results/Table_2_", format(Sys.Date(), format = "%Y%m%d"),".csv"))
...
# Read in csv to then append adjusted pvalues
write.csv(tab2_2_padjust, paste0("./Results/Table_2_padjust_", format(Sys.Date(), format = "%Y%m%d"),".csv"), row.names = TRUE)
```
...
cutpoints_unnest %>% 
  ...
  mutate(optimal_cutpoint = case_when(compound == "deoxycholic acid" ~ (optimal_cutpoint / deoxycholic_mw)*1000,
         ...
         `0` = paste(">=", optimal_cutpoint, "µM"),
         `1` = paste("<", optimal_cutpoint, "µM")) %>% 
  select(compound, `0`,`1`) %>% 
  mutate(compound = as.factor(compound),
         compound = factor(compound, levels = c("Deoxycholic Acid", "Isodeoxycholic Acid", "Lithocholic Acid",  "Desaminotyrosine"))) %>% 
  arrange(compound) %>% 
write.csv(., paste0("./Results/Table_3_", format(Sys.Date(), format = "%Y%m%d"), ".csv"), row.names = F)
```
...
gtsave(as_gt(coxauc),
       file = paste0("./Results/Table_4_", format(Sys.Date(), format = "%Y%m%d"), ".png"))
```
...
gtsave(as_gt(log_t5_1),
       file = paste0("./Results/Table_5_", format(Sys.Date(), format = "%Y%m%d"), ".png"))
```
...
write.csv(tab_s1_2, paste0("./Results/Table_S1_", format(Sys.Date(), format = "%Y%m%d"),".csv"), row.names = TRUE)

# Need to adjust pvalues
tab_s1_2_padjust <- read.csv(paste0("./Results/Table_S1_", format(Sys.Date(), format = "%Y%m%d"),".csv"))
...
# Read in csv to then append adjusted pvalues
write.csv(tab_s1_2_padjust, paste0("./Results/Table_5_padjust_", format(Sys.Date(), format = "%Y%m%d"),".csv"), row.names = TRUE)
```
...
ggsave(plot = gg_emapper_box, file = paste0("./Results/Figure_S2_",format(Sys.Date(), format = "%Y%m%d"), ".pdf"), width = 14, height = 10, units = "in")
```
...
pdf(paste0("./Results/Figure_S3_", format(Sys.Date(), format = "%Y%m%d"),".pdf"), height = 12, width = 17, onefile = F)
gg_covid_heatmap
dev.off()
...
save.image(file = paste0("SARS-CoV-2_Modeling_", format(Sys.Date(), format = "%Y%m%d"), ".RData"))
```
